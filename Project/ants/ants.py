...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        i = self.place
        step = 0
        while i.is_hive == False:
            if i.bees:
                if self.lower_bound <= step and step <= self.upper_bound:
                    return random_bee(i.bees)
            i = i.entrance
            step += 1
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        """
        # BEGIN Problem 5
        current_place = self.place  #防止下一行血量归零被直接移除
...
# The WallAnt class
class WallAnt(Ant):
    """WallAnt is an ant that has a large amount of health but does not do
    anything each turn.
    """
...
# The HungryAnt Class
class HungryAnt(Ant):
    """HungryAnt eats a Bee in its Place every three turns."""
    ...
    def action(self, gamestate):
        """Eat a Bee in the Ant's Place if not digesting."""
        # BEGIN Problem 7
        if self.cooldown > 0:
            self.cooldown -= 1
        else:
...
class TankAnt(ContainerAnt):
    """TankAnt is a container ant that also deals damage to all bees in its place."""
    ...
    def action(self, gamestate):
        """Deal damage to all bees in the TankAnt's place."""
        # BEGIN Problem 9
...
class ScubaThrower(ThrowerAnt):
    """ScubaThrower is a waterproof ThrowerAnt."""
