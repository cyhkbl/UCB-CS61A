...
def accuracy(typed, source):
    ...
    if len(typed_words) == 0 and len(source_words) == 0:
        ...
        if len(typed_words) > len(source_words):
            incorrect += abs(len(typed_words) - len(source_words))
        for word_index in range(min(len(typed_words), len(source_words))):
            if typed_words[word_index] == source_words[word_index]:
                correct += 1
            else:
                incorrect += 1
...
def memo_diff(diff_function):
    cache = {}  # key: (typed, source) -> (best_limit, result_at_best_limit)

    def memoized(typed, source, limit):
        key = (typed, source)

        if key in cache:
            cached_limit, cached_result = cache[key]
            # 如果之前算过的limit >= 现在的limit，直接复用
            if cached_limit >= limit:
                return cached_result

        # 否则需要重新计算，并把“最大的limit结果”缓存起来
        result = diff_function(typed, source, limit)
        cache[key] = (limit, result)
        return result
...
def furry_fixes(typed, source, limit):
    ...
    """
    # BEGIN PROBLEM 6
    def count(object, source, diff, limit):
        if diff > limit:
            return diff
        if len(object) == 0 or len(source) == 0:
            return diff + abs(len(object) - len(source))
        if object[0] != source[0]:
            return count(object[1:], source[1:], diff + 1, limit)
        else:
            return count(object[1:], source[1:], diff, limit)
    return count(typed, source, 0, limit)
    # END PROBLEM 6
...
def report_progress(typed, source, user_id, upload):
    ...
    # BEGIN PROBLEM 8
    correct = 0
    for words_index in range(len(typed)):
        if typed[words_index] == source[words_index]:
            correct += 1
        else:
            break
    progress = correct / len(source)
    person = {'id': user_id, 'progress': progress}
    upload(person)
    return progress
    # END PROBLEM 8
...
def time_per_word(words, timestamps_per_player):
    ...
    for i in range(len(tpp)):
        ...
        for j in range(len(tpp[i]) - 1):
            templist.append(tpp[i][j + 1] - tpp[i][j])
    ...
    for i in word_indices:
        ...
        for j in player_indices:
            if times[j][i] < times[fastest_player][i]:
                fastest_player = j
        player_fastest[fastest_player].append(words[i])
